...
function U($url,$vars='',$suffix=true,$redirect=true,$domain=false) {
    // 解析URL
    $info =  parse_url($url);
...
function F($name, $value='', $path=DATA_PATH) {
    ...
    if ('' !== $value) {
        if (is_null($value)) {
            ...
            // 目录不存在则创建
			$fileStoreService->createDir($dir);
            $_cache[$name] =   $value;
            return $fileStoreService->saveTextFile(serialize($value),$filename);
        }
    }
    ...
    if (isset($_cache[$name]))
        return $_cache[$name];
    // 获取缓存数据
    if ($fileStoreService->isFileExist($filename)) {
        $value = my_unserialize( $fileStoreService->getFileText($filename) );
        $_cache[$name] = $value;
    } else {
        $value = false;
    }
